...
export function Stepper({ value, onChange, min = 0, max = Infinity, step = 1, suffix, className }: StepperProps) {
  ...
  return (
    <div className={cn('flex h-10 items-stretch overflow-hidden rounded-lg border border-border-default bg-surface', className)}>
      <button
        type="button"
        aria-label="Diminuir"
        onClick={() => onChange(clamp(value - step))}
        className="flex w-10 items-center justify-center text-fg-muted hover:bg-surface-subtle disabled:opacity-40"
        disabled={value <= min}
      >
        <Minus size={16} />
      </button>
