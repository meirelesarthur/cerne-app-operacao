...
void main() {
  group('AppButton', () {
    testWidgets('dispara onPressed ao tocar', (tester) async {
      var tapped = false;
      await tester.pumpWidget(_wrap(AppButton(onPressed: () => tapped = true, child: const Text('OK'))));
    ...
    testWidgets('loading=true ignora toque e mostra o spinner', (tester) async {
      var tapped = false;
      await tester.pumpWidget(
        _wrap(AppButton(loading: true, onPressed: () => tapped = true, child: const Text('Loading'))),
      );
    ...
    testWidgets('renderiza todas as variantes e tamanhos sem exceções', (tester) async {
      for (final variant in AppButtonVariant.values) {
        for (final size in AppButtonSize.values) {
          await tester.pumpWidget(
            _wrap(AppButton(variant: variant, size: size, onPressed: () {}, child: const Text('X'))),
          );
