...
void main() {
  group('AppErrorState', () {
    testWidgets('renderiza título e descrição padrão', (tester) async {
      ...
      expect(find.text('Ocorreu um erro ao buscar os dados. Tente novamente.'), findsOneWidget);
      expect(find.text('Tentar novamente'), findsNothing);
    });

    testWidgets('mostra botão de retry e dispara onRetry ao tocar', (tester) async {
      var retried = false;
      await tester.pumpWidget(_wrap(AppErrorState(onRetry: () => retried = true)));
    ...
    testWidgets('aceita título e descrição customizados', (tester) async {
      await tester.pumpWidget(
        _wrap(const AppErrorState(title: 'Falha ao sincronizar', description: 'Verifique sua conexão.')),
      );
