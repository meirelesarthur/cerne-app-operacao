...
GoRouter _router(VoidCallback onExit) => GoRouter(
  ...
  routes: [
    GoRoute(path: '/bank/pagamentos', builder: (context, state) => PixFlow(onExit: onExit)),
...
Widget _wrap(VoidCallback onExit) => ProviderScope(
  child: MaterialApp.router(theme: buildAppTheme(AppThemeVariant.light), routerConfig: _router(onExit)),
);

void main() {
  group('PixFlow', () {
    testWidgets('renderiza o passo 1 (chave) com contatos frequentes', (tester) async {
    ...
    testWidgets('fluxo completo: contato → valor → revisão → sucesso', (tester) async {
      await setTallSurface(tester);
      await tester.pumpWidget(_wrap(() {}));
      await tester.pumpAndSettle();
    ...
    testWidgets('valor vazio mantém "Revisar" desabilitado (toque não avança)', (tester) async {
      await setTallSurface(tester);
      await tester.pumpWidget(_wrap(() {}));
      await tester.pumpAndSettle();
