...
const _loteCabecas = <String, int>{'l42': 128, 'l19': 96, 'l07': 150, 'l33': 64, 'l51': 110, 'l88': 82};
...
class _VendaFlowState extends ConsumerState<VendaFlow> {
  ...
  bool get _totalValido => (double.tryParse(_total.replaceAll(',', '.')) ?? 0) > 0;
  bool get _valid =>
      _lote != null && _cliente.isNotEmpty && _cond != null && _dataEmbarque.isNotEmpty && _contagemBate && _totalValido;
  ...
  void _confirmar() {
    ...
    if (queued) {
      ref.read(fazendasStoreProvider.notifier).enqueueSync(
            SyncItem(id: 'venda-$_lote', label: 'Venda de animais', detail: _cliente, kind: ActivityKind.venda),
          );
    }
      ...
      child: Column(
        ...
        children: [
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(shape: BoxShape.circle, color: semantic.bgSurface),
                    child: Icon(LucideIcons.lock, size: 16, color: semantic.fgSubtle),
                  ),
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(fontFamily: AppTypography.fontFamily, fontWeight: AppTypography.weightMedium, color: semantic.fgMuted),
                        ),
                        Text(
          ...
          AppFormField(
            ...
            required: true,
            error: !_contagemBate ? 'A contagem deve bater com o lote ($_esperado cabeças).' : null,
            child: AppStepper(value: _qtd, onChanged: (v) => setState(() => _qtd = v), max: _esperado > 0 ? _esperado : 999),
          ),
          const SizedBox(height: AppSpacing.space5),
          AppFormField(
            label: 'Cliente',
            required: true,
            child: AppTextInput(onChanged: (v) => setState(() => _cliente = v), placeholder: 'Nome do comprador'),
          ),
          const SizedBox(height: AppSpacing.space5),
          AppFormField(
          ...
          AppFormField(
            ...
            required: true,
            child: AppTextInput(onChanged: (v) => setState(() => _dataEmbarque = v), placeholder: 'dd/mm/aaaa'),
          ),
          const SizedBox(height: AppSpacing.space5),
          AppFormField(
            label: 'Valor total (R\$)',
            required: true,
            error: _total.isNotEmpty && !_totalValido ? 'O total deve ser maior que zero.' : null,
            child: AppTextInput(
